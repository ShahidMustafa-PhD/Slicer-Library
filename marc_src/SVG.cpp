...
#define COORD(x) ((float)unscale((x))*10)   //  scaled to 0.1mm.
...
bool SVG::open(const char* afilename)
{
	...
	this->origin   = Point(scale_(-100),scale_(-100));
...
{
    this->filename = afilename;
    this->origin   = bbox.min - Point(bbox_offset, bbox_offset); //slm
    this->flipY    = aflipY;
    this->f        = ::fopen(afilename, "w");
    if (f == NULL)
        return false;
    ...
    fprintf(this->f,
        "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n"
        "<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 1.0//EN\" \"http://www.w3.org/TR/2001/REC-SVG-20010904/DTD/svg10.dtd\">\n"
        "<svg height=\"%fmm\" width=\"%fmm\" viewBox=\"0 0 %f %f\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:svg=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n"
