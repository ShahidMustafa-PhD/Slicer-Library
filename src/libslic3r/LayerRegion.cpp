...
LayerRegion::make_perimeters(const SurfaceCollection &slices, SurfaceCollection* fill_surfaces)
{
    ...
    PerimeterGenerator g(
       ...
       // &this->layer()->object()->print()->config,
        this->layer()->object()->print()->jconfig,
        // output:
        &this->perimeters,
        &this->thin_fills,
        fill_surfaces
...
LayerRegion::process_external_surfaces()
{
    ...
    for (const Surface &surface : surfaces) {
        ...
        if (this->layer()->lower_layer != NULL && surface.is_bridge()) {
            ...
            if (bd.detect_angle()) {
                ...
                if (this->layer()->object()->jconfig.support_material) {
    ...
    if (this->region()->jconfig.fill_density > 0) {
...
LayerRegion::prepare_fill_surfaces()
{
    ...
    double min_top_bottom_shell_thickness=0.16;
    // if no solid layers are requested, turn top/bottom surfaces to internal
    if (1 && min_top_bottom_shell_thickness <= 0) {
        for (Surface &surface : this->fill_surfaces.surfaces) {
            if (surface.surface_type == stTop) {
                if (false) { //this->layer()->object()->jconfig.infill_only_where_needed
    ...
    const float &fill_density = this->region()->jconfig.fill_density;
    if (fill_density > 0 && fill_density < 100) {
        // scaling an area requires two calls!
        // (we don't use scale_() because it would overflow the coord_t range
        const double min_area = 10.0 / SCALING_FACTOR / SCALING_FACTOR;
        for (Surface &surface : this->fill_surfaces.surfaces) {
