...
Print::max_allowed_layer_height() const
{
   ...
   // std::set<size_t> extruders = this->extruders();
    ...
    return jconfig.layer_thickness;//*std::max_element(nozzle_diameter.begin(), nozzle_diameter.end());
}
   ...
   // o->config.apply(this->default_object_config);
    ...
    // update placeholders
    {
        ...
        if (!input_file.empty()) {
            ...
            const std::string input_basename = std::filesystem::path(input_file).filename().string();
...
Print::export_gcode(std::ostream& output, bool quiet)
{
    ...
    // disabled by shahid... to make library lighter...
    //Slic3r::PrintGCode(*this, output).output();// This is the final reult
}
        ...
        // check horizontal clearance
        {
            ...
            FOREACH_OBJECT(this, i_object) {
                ...
                convex_hull = offset(
                    ...
                    scale_(jconfig.hatch_spacing)/2 - scale_(0.001),
...
Print::brim_flow() const
{
    ...
    if (width.value == 0) width = this->regions.front()->jconfig.perimeter_extrusion_width;
    if (width.value == 0) width = this->objects.front()->jconfig.extrusion_width;
   ...
   // Flow flow = Flow::new_from_config_width(
        //frPerimeter,
       // width, 
        //this->jconfig.nozzle_diameter.get_at(this->regions.front()->jconfig.perimeter_extruder-1),
...
Print::output_filepath(const std::string &path)
{
    ...
    if (path.empty()) {
        ...
        return (std::filesystem::path(input_file).parent_path() / this->output_filename()).string();
    }
...
void Print::apply_slm_config(const SlmConfig& slm_config) {
    try {
        // Perform a deep copy of the SlmConfig
        // Since SlmConfig only contains primitive types, a direct assignment works
        this->jconfig = slm_config;
        
        // Apply configuration to all PrintObjects
        for (PrintObject* obj : this->objects) {
            if (obj == nullptr) continue;
            
            // Copy the config to each PrintObject
            obj->jconfig = slm_config;
            
            // Apply to all regions of this object
            for (size_t region_id = 0; region_id < this->regions.size(); ++region_id) {
                PrintRegion* region = this->get_region(region_id);
                if (region == nullptr) continue;
                
                // Copy the config to each PrintRegion
                region->jconfig = slm_config;
            }
        }
        
        //std::cout << "Successfully applied SLM configuration to print and all objects/regions" << std::endl;
        
        // Invalidate all steps to ensure the configs take effect
        this->invalidate_all_steps();
    } catch (const std::exception& e) {
        std::cerr << "Error applying SLM configuration: " << e.what() << std::endl;
    }
}
