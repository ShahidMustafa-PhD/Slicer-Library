...
class PrintObject
{
  ...
  //  bool invalidate_state_by_config(const PrintConfigBase &config);
    ...
    ~PrintObject();
 struct SpPillar {
    Point position;        // XY position
    size_t top_layer;      // Index of top layer
    size_t bottom_layer;   // Index of bottom layer
    double radius_base;    // Base radius
    double radius_top;     // Top radius (for tapered pillars)
};  SpPillar  pillar;
// Store overhang areas by region and layer
std::map<size_t, std::map<size_t, ExPolygons>> overhang_areas;
    ...
    void detect_overhangs(std::map<size_t, std::map<size_t, ExPolygons>>& overhang_areas, const double support_angle);
    void generate_pillar_positions(const std::map<size_t, std::map<size_t, ExPolygons>>& overhang_areas,
    std::map<size_t, Points>& pillar_positions,const coordf_t pillar_spacing);
    void create_support_pillars(const std::map<size_t, Points>& pillar_positions,const coordf_t pillar_size);
    void generate_support_geometry(const std::map<size_t, std::vector<SpPillar>>& pillars_by_region,const coordf_t pillar_base_size);
...
class Print
{
    public:
    SlmConfig jconfig;
    void apply_slm_config(const SlmConfig& slm_config);
    //PrintConfig config;
