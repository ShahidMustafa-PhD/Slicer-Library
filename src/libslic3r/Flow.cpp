...
Flow::_auto_width(FlowRole role, float nozzle_diameter, float height) {
    ...
    // for slm  nozzle diameter=hatchspacing  and height=hatch spacing.
    return width;
    float min = nozzle_diameter;// * 1.05;  /for slm
...
Flow::_width_from_spacing(float spacing, float nozzle_diameter, float height, bool bridge) {
    ...
    // rectangle with semicircles at the ends
    return spacing ;//+ OVERLAP_FACTOR * height * (1 - PI/4.0); // for slm
}
