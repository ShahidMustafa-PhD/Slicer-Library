...
//#else
//#include <boost/nowide/cenv.hpp>  // not needed with boost 1.81 or later.. shahid 18/04/2025
//#endif
    ...
    for (auto category : categories) {
        ...
        for (const auto& opt : this->options) {
            ...
            if (!def.cli.empty()) {
               ...
               //out << " " << std::left << std::setw(20) << cli;  // by shahid
...
ConfigBase::load(const std::string &file)
{
    ...
    try {
        ...
        std::cout << "Error reading ini file: " << file << ": " << ex.what() << std::endl;
    ...
    BOOST_FOREACH(const pt::ptree::value_type &v, tree) {
        try {
            ...
            // ignore
            std::cout << "Unknown option in config file: " << e.what() << std::endl;
        }
    }
}
