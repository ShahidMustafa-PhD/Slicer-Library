...
class LayerRegion
{
    ...
    //slm data 
    SurfaceCollection fill_surfaces_support;
	//Polylines fill_slm;
    ...
    std::vector<SlmHatch> hatches;
	std::vector<Marc::ScanSegmentHatch> segment_hatches_;// newly created data structure for slm hatches
    std::vector<Marc::ScanSegmentPolyline> segment_polylines_;// newly created data structure for slm plylines
    // slm data ends here
    ...
    void make_hatches(const Surface &surface);
    Lines generate_fill_lines(const BoundingBox& bbox ) const;
    private:
    ...
    mutable std::mutex _slices_mutex;
