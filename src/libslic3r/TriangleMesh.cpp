...
#include "SVG.hpp"
#endif
//#include <lib3mf_implicit.hpp> // Include the lib3mf header
namespace Slic3r {
...
bool TriangleMesh::orient_optimum_rotation()
{
    // Create optimizer for this mesh
   // auto optimizer = std::make_unique<MeshOrientationOptimizer>(*this);
   ...
   // float roll = 0.0f, pitch = 0.0f, yaw = 0.0f;
    //optimizer->get_best_angles(roll, pitch, yaw);

    // Optionally apply the rotation to this mesh here, if needed
...
//Added By Shahid 24/4/25
//#include <lib3mf_implicit.hpp> // Include the lib3mf header
//#include "TriangleMesh.hpp"
/*
void TriangleMesh::Read3MFFile(const std::string &input_file) {
    // Create a 3MF reader
    ...
    // Load the 3MF file
    reader->ReadFromFile(input_file);

    // Access the mesh objects in the 3MF file
    auto iterator = model->GetMeshObjects();
    if (!iterator->MoveNext()) {
        throw std::runtime_error("No mesh objects found in the 3MF file.");
    }

    // Initialize the TriangleMesh
    stl_initialize(&this->stl);
    this->stl.stats.type = inmemory;
    ...
    while (iterator->MoveNext()) {
        ...
        this->stl.stats.number_of_facets = triangles.size();
        this->stl.stats.original_num_facets = this->stl.stats.number_of_facets;
        stl_allocate(&this->stl);

        // Copy vertices and triangles into the TriangleMesh
        for (size_t i = 0; i < triangles.size(); ++i) {
            const auto &triangle = triangles[i];
            stl_facet &facet = this->stl.facet_start[i];

            facet.vertex[0].x = vertices[triangle.Indices[0]].m_Coordinates[0];
            facet.vertex[0].y = vertices[triangle.Indices[0]].m_Coordinates[1];
            facet.vertex[0].z = vertices[triangle.Indices[0]].m_Coordinates[2];

            facet.vertex[1].x = vertices[triangle.Indices[1]].m_Coordinates[0];
            facet.vertex[1].y = vertices[triangle.Indices[1]].m_Coordinates[1];
            facet.vertex[1].z = vertices[triangle.Indices[1]].m_Coordinates[2];

            facet.vertex[2].x = vertices[triangle.Indices[2]].m_Coordinates[0];
            facet.vertex[2].y = vertices[triangle.Indices[2]].m_Coordinates[1];
            facet.vertex[2].z = vertices[triangle.Indices[2]].m_Coordinates[2];
