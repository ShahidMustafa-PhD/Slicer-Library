...
ModelObject::cut(Axis axis, coordf_t z, Model* model) const
{
    ...
    if (this->input_file.empty()) {
        ...
        const std::filesystem::path p{this->input_file};
...
ModelObject::print_info() const
{
    using namespace std;
    cout << fixed;
    cout << "[" << std::filesystem::path(this->input_file).filename().string() << "]" << endl;
...
// added by Shahid 22/4/25
bool 
ModelObject::arrange_volumes(coordf_t dist, const BoundingBoxf* bb)
{
    // get the (transformed) size of each instance so that we take
    // into account their different transformations when packing
        //coordf_t dist=5;
	    Pointfs volume_sizes;
		Pointfs positions;
		BoundingBoxf3 bb1;
		//const BoundingBoxf* bb= new  BoundingBoxf(Pointf(0,0),Pointf(100,100));
		// BoundingBox  BBB=  BoundingBox((Point(scale_(-25),scale_(-25)), Point(scale_(25),scale_(25))));
		// Points  BBB << Point(scale_(25),scale_(0))<< Point(scale_(-25),scale_(0)) << Point(scale_(0),scale_(25));
	for (const ModelVolume* v : this->volumes)
	{if (!v->modifier)
		  bb1=v->mesh.bounding_box();
	volume_sizes.push_back(bb1.size());}
	     
    if (! this->_arrange(volume_sizes,dist, bb, positions))
        return false;
    
   for (ModelVolumePtrs::const_iterator v = this->volumes.begin(); v != this->volumes.end(); ++v) {
        (*v)->mesh.center_around_origin();//-positions.back().x,-positions.back().y,0);
        Pointf p=positions.back();
		bb1=(*v)->mesh.bounding_box();
		(*v)->mesh.translate(p.x,p.y,-bb1.min.z);
        (*v)->mesh.build_plate_position;// save the position of the mesh in the plate
        
        positions.pop_back();
        }
       //(*v)->invalidate_bounding_box(); //    enabled on 21April...
    return true;
	}
// added 22/4/25
bool
ModelObject::_arrange(const Pointfs &sizes, coordf_t dist, const BoundingBoxf* bb, Pointfs &out) const
    {
        // we supply unscaled data to arrange()
        bool result = Slic3r::Geometry::arrange(
            sizes.size(),               // number of parts
...
bool ModelVolume::optimize_mesh_orientations(float& roll, float& pitch) 
{
    // Parallelize the loop since each task (volume optimization) is independent
   
   // for (int i = 0; i < static_cast<int>(this->volumes.size()); ++i) {
       
        auto optimizer = std::make_unique<MeshOrientationOptimizer>(this->mesh);

        // Optimize mesh orientation
        //std::cout  << "Entering optimizer2....." <<std::endl;
        optimizer->optimize();
        //std::cout  << "Exiting optimizer2....." <<std::endl;
      
        optimizer->get_best_angles(roll, pitch);

        // Optionally apply the rotation to the mesh
        this->mesh.rotate_x(roll);
        this->mesh.rotate_y(pitch);

        // Store the best roll and pitch values
        this->best_roll=roll;
        this->best_pitch=pitch;
       // v->mesh.rotate_z(yaw);
    //}
