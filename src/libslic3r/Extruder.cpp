...
Extruder::retract(double length, double restart_extra)
{
    ...
    double to_retract = -1;//length - this->retracted;
    if (to_retract > 0) {
        this->E -= to_retract;
...
{
    return 0.1;//this->jconfig->filament_diameter;//.get_at(this->id);
}

double
Extruder::filament_density() const
{
    return 100;//this->config->filament_density.get_at(this->id);
}

double
Extruder::filament_cost() const
{
    return 2.0;//this->config->filament_cost.get_at(this->id);
}

double
Extruder::extrusion_multiplier() const
{
    return 1.0;//this->config->extrusion_multiplier.get_at(this->id);
}

double
Extruder::retract_length() const
{
    return 1;//this->config->retract_length.get_at(this->id);
}

double
Extruder::retract_lift() const
{
    return 1;//this->config->retract_lift.get_at(this->id);
}

int
Extruder::retract_speed() const
{
    return 1;//this->config->retract_speed.get_at(this->id);
}

double
Extruder::retract_restart_extra() const
{
    return 1;//this->config->retract_restart_extra.get_at(this->id);
}

double
Extruder::retract_length_toolchange() const
{
    return 1;//this->config->retract_length_toolchange.get_at(this->id);
}

double
Extruder::retract_restart_extra_toolchange() const
{
    return 1;//this->config->retract_restart_extra_toolchange.get_at(this->id);
}
