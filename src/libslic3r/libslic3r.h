...
constexpr auto RESOLUTION = 0.00125;
constexpr auto SCALED_RESOLUTION = scale_(RESOLUTION);
constexpr auto PI = 3.141592653589793238;
// When extruding a closed loop, the loop is interrupted and shortened a bit to reduce the seam.
constexpr auto LOOP_CLIPPING_LENGTH_OVER_NOZZLE_DIAMETER = 0.015;
constexpr coord_t SMALL_PERIMETER_LENGTH = scale_(6.5) * 2 * PI;
constexpr coordf_t INSET_OVERLAP_TOLERANCE = 0.04;
constexpr coordf_t EXTERNAL_INFILL_MARGIN = 1;//3;
...
_parallelize_do(std::queue<T>* queue, std::mutex* queue_mutex, std::function<void(T)> func)
{
    ...
    int threads_count = std::thread::hardware_concurrency())
{
    ...
    int num_threads = std::min(threads_count, static_cast<int>(queue.size()));

    for (int i = 0; i < num_threads; i++)
        workers.emplace_back(_parallelize_do<T>, &queue, &queue_mutex, func);
