...
PerimeterGenerator::process()
{
    ...
    coord_t pwidth              = scale_(this->jconfig.hatch_spacing);//this->perimeter_flow.scaled_width();
    coord_t pspacing            = scale_(this->jconfig.hatch_spacing);// this->perimeter_flow.scaled_spacing();

    // external perimeters
    this->_ext_mm3_per_mm       = this->ext_perimeter_flow.mm3_per_mm();
    coord_t ext_pwidth          = scale_(this->jconfig.hatch_spacing);//this->ext_perimeter_flow.scaled_width();
    coord_t ext_pspacing        = scale_(this->jconfig.hatch_spacing);// this->ext_perimeter_flow.scaled_spacing();
    coord_t ext_pspacing2       = scale_(this->jconfig.hatch_spacing);//this->ext_perimeter_flow.scaled_spacing(this->perimeter_flow);
   ...
   // double min_shell_thickness = 0.16;
    coord_t min_shell_thickness = scale_(0.16);
    
    // prepare grown lower layer slices for overhang detection
    if (this->lower_slices != NULL && this->jconfig.overhangs) {
        // We consider overhang any part where the entire nozzle diameter is not supported by the
        // lower layer, so we take lower slices and offset them by half the nozzle diameter used 
        // in the current layer
        
        //for SLM printing we use hatch_spacing as nozzle_diameter
        double nozzle_diameter = this->jconfig.hatch_spacing;//this->print_config->nozzle_diameter.get_at(this->config->perimeter_extruder-1);
    ...
    for (Surfaces::const_iterator surface = this->slices->surfaces.begin();
        ...
        int loops = this->jconfig.perimeters + surface->extra_perimeters;
        ...
        if (loop_number >= 0) {  // no loops = -1
            ...
            for (int i = 0; i <= loop_number+1; ++i) {  // outer loop is 0
                ...
                if (i == 0) {
                    ...
                    // look for gaps
                    if (this->jconfig.fill_gaps && this->jconfig.fill_density> 0) {
            ...
            if (this->jconfig.external_perimeters_first)
    ...
    ThickPolylines &thin_walls) const
{
    ...
    for (PerimeterGeneratorLoops::const_iterator loop = loops.begin();
        ...
        if (this->jconfig.overhangs && this->layer_id > 0
            && !(this->jconfig.support_material && this->jconfig.support_material_model_clearance == 0)) {
