...
PrintRegion::flow(FlowRole role, double layer_height, bool bridge, bool first_layer, double width, const PrintObject &object) const
{
    ...
    if (width != -1) {
      ...
      config_width = this->_print->jconfig.hatch_spacing;
    if (config_width == 0) {
    ...
    double nozzle_diameter = this->_print->jconfig.hatch_spacing;
    
    //static Flow new_from_spacing(float spacing, float nozzle_diameter, float height, bool bridge);
    return Flow::new_from_spacing(nozzle_diameter, nozzle_diameter, layer_height, bridge);
}
