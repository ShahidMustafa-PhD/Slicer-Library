...
#include <omp.h> // Include if not already present
namespace Slic3r {
...
class ModelObject
{
    ...
    void print_info() const;
    bool arrange_volumes(coordf_t dist, const BoundingBoxf* bb); 
    bool _arrange(const Pointfs &sizes, coordf_t dist, const BoundingBoxf* bb, Pointfs &out) const;
    private:
...
class ModelVolume
{
   ...
   float best_roll, best_pitch;
    // -added  25/05/2025
    // -this method applies the optimizations
    // -based on minimum volume support structure
 bool optimize_mesh_orientations(float& roll, float& pitch);
