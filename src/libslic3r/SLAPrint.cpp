...
#include <complex>
#include <cstdio>
//#include "../MarcSlice/MarcFile.hpp"
//#include "../MarcSlice/ExportMarcSlice.hpp"
#include <memory>
...
void
SLAPrint::slice()
{   //-mesh() dosnt work raw_mesh() works --sm
    //TriangleMesh mesh = this->model->mesh();
    TriangleMesh mesh = this->model->raw_mesh();
  
    std::cout << "STL Number of Facets1:" << mesh.facets_count() <<std::endl;
    mesh.repair();
    std::cout << "STL Number of Facets2:" << mesh.facets_count() <<std::endl;
    mesh.align_to_bed();
    ...
    const float first_lh = this->config.first_layer_height.value;
     //-sm
     std::cout << "The  Layer Hieght is:" << lh <<std::endl;
     //-sm
    ...
    std::cout << "Max STL Z:" << mesh.stl.stats.max.z <<std::endl;
    ...
    //-sm
    std::cout << "The  number of layers are:" <<this->layers.size() <<std::endl;
    //-sm
    // generate infill
    if (this->config.fill_density < 100) {
        ...
        std::cout << "Fill-> min spacing:" <<fill->min_spacing  <<std::endl;
...
// For example, return absolute infill extrusion width, either from an absolute value, or relative to a provided value.
    ...
    //std::cout << "Shell Thickness:" <<shell_thickness  <<std::endl;
