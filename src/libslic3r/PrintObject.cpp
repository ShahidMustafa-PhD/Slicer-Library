...
{
    return this->jconfig.support_material
        || this->jconfig.anchors > 0;
...
PrintObject::detect_surfaces_type()
{
    ...
    // prerequisites
    //this->slice();  // done inside make_perimeters()
    
   /* parallelize<Layer*>(
        std::queue<Layer*>(std::deque<Layer*>(this->layers.begin(), this->layers.end())),  // cast LayerPtrs to std::queue<Layer*>
        std::bind(&Slic3r::Layer::detect_surfaces_type, _1),
        this->_print->config.threads.value
    );*/
    parallelize<Layer*>(
    std::queue<Layer*>(std::deque<Layer*>(this->layers.begin(), this->layers.end())),
    [](Layer* layer) { layer->detect_surfaces_type(); },
    this->jconfig.threads
);
...
PrintObject::process_external_surfaces()
{
    /*parallelize<Layer*>(
        std::queue<Layer*>(std::deque<Layer*>(this->layers.begin(), this->layers.end())),  // cast LayerPtrs to std::queue<Layer*>
        std::bind(&Slic3r::Layer::process_external_surfaces, _1),
        this->_print->config.threads.value
    );*/
    parallelize<Layer*>(
    std::queue<Layer*>(std::deque<Layer*>(this->layers.begin(), this->layers.end())),
    [](Layer* layer) { layer->process_external_surfaces(); },
    this->jconfig.threads
);
...
PrintObject::bridge_over_infill()
{
    FOREACH_REGION(this->_print, region) {
        ...
        if ((*region)->jconfig.fill_density == 100) continue;
        ...
        FOREACH_LAYER(this, layer_it) {
            ...
            // check whether we should bridge or not according to density
            {
               ...
               // const Flow normal_flow = layerm->flow(frSolidInfill, false);
                ...
                const float density_threshold = 50; //std::max(
                    //std::min<float>(
                        //min_threshold
               ...
               //);
                ...
                if ((*region)->jconfig.fill_density > density_threshold) continue;
            }
            ...
            ExPolygons to_bridge;
            {
                ...
                for (int i = (layer_it - this->layers.begin()) - 1; i >= 0; --i) {
                  ...
                  //  excess_mm3_per_mm2 -= lower_layer->height * (100 - (*region)->jconfig.fill_density)/100;
                ...
                {
                    const double min_width = jconfig.layer_thickness*3;//.scaled_width() * 3;
...
coordf_t PrintObject::adjust_layer_height(coordf_t layer_height) const
{
    ...
    if(this->jconfig.z_steps_per_mm > 0) {
        coordf_t min_dz = 1 / this->jconfig.z_steps_per_mm;
...
std::vector<coordf_t> PrintObject::generate_object_layers(coordf_t first_layer_height) {
   ...
   // slm settings
   //this->config.layer_height.getFloat();
   ...
   // collect values from config
    ...
    coordf_t layer_height= this->jconfig.layer_thickness;// enforced for slm
     min_nozzle_diameter = layer_height;// enforced for slm
     min_layer_height = 0.98*layer_height;// enforced for slm
     max_layer_height = 1.02*layer_height;// enforced for slm
     bool adaptive_slicing= false; // enforced for slm
    layer_height = this->adjust_layer_height(layer_height);
    this->jconfig.layer_thickness = layer_height;
    ...
    if (this->state.is_done(posLayers)) {
        ...
        while ((scale_(print_z + EPSILON)) < this->size.z) {
            ...
            if (0) {
                ...
                if(this->jconfig.match_horizontal_surfaces) {
        ...
        if ((this->jconfig.z_steps_per_mm > 0 || this->jconfig.adaptive_slicing) && result.size() > 1) {
            ...
            if (diff < 0) {
                ...
                if(this->jconfig.adaptive_slicing) { // use min/max layer_height values from adaptive algo.
    ...
    if(this->jconfig.z_steps_per_mm > 0) {
        coordf_t gradation = 1 / this->jconfig.z_steps_per_mm;
        coordf_t last_z = 0;
        coordf_t height;
...
void PrintObject::_slice()
{
    ...
    coordf_t first_layer_height = this->jconfig.first_layer_thickness;//.get_abs_value(this->config.layer_height.value);


    // take raft layers into account
    int id = 0;
    //this->config.raft_layers=0;  // for slm
    if (this->jconfig.anchors > 0) {

        id = static_cast<int>(this->jconfig.anchors/this->jconfig.anchors_layer_thickness);

        coordf_t min_support_nozzle_diameter = this->jconfig.hatch_spacing*1.25;
       ...
       // }
        coordf_t support_material_layer_height = this->jconfig.anchors_layer_thickness; // 0.75 * min_support_nozzle_diameter;

        // raise first object layer Z by the thickness of the raft itself
        // plus the extra distance required by the support material logic
        raft_height += first_layer_height;
        raft_height += this->jconfig.anchors;//support_material_layer_height * (this->config.raft_layers - 1);
      ...
      raft_height = 0;  // we will take care of support layers in the add_anchors ()
    ...
    std::vector<float> slice_zs;
    {
       ...
       // coordf_t slice_z);  print_z is the actual hieght from whaere printing starts in the machine. 
      // below this are the anchor layers
    ...
    parallelize<Slic3r::SurfaceCollection*>(
    queue,
    [](Slic3r::SurfaceCollection* sc) { sc->remove_collinear_points(); },
    this->jconfig.threads
    );


    // Apply size compensation and perform clipping of multi-part objects.
    const coord_t xy_size_compensation = scale_(this->jconfig.xy_size_compensation);
    for (Layer* layer : this->layers) {
        ...
        if (this->jconfig.regions_overlap > 0) {
            const coord_t delta = scale_(this->jconfig.regions_overlap)/2;
            for (LayerRegion* layerm : layer->regions)
                layerm->slices.set(
                    intersection_ex(
...
PrintObject::make_perimeters()
{
    ...
    this->add_anchor_layers(); // after cleanly slicing, we can add anchor layers

   if (this->layers.empty()) {
        Slic3r::Log::error("PrintObject") << "make_perimeters(): " << "No layers were detected. You might want to repair your STL file(s) or check their size or thickness and retry.\n";
        return; // make this throw an exception instead?
    }

    // if we don't have any perimeters, we don't need to do anything
    if (this->jconfig.perimeters == 0) {
   ...
   // return ;  // for slm we don't need to make perimeters
    ...
    FOREACH_REGION(this->_print, region_it) {
        ...
        if (!jconfig.extra_perimeters
            || jconfig.perimeters == 0
            || jconfig.fill_density == 0
        ...
        for (size_t i = 0; i <= (this->layer_count()-2); ++i) {
            ...
            const coord_t perimeter_spacing     =scale_(jconfig.hatch_spacing );//layerm.flow(frPerimeter).scaled_spacing();
            const Flow ext_perimeter_flow       = layerm.flow(frExternalPerimeter);
            const coord_t ext_perimeter_width   = scale_(jconfig.hatch_spacing );//ext_perimeter_flow.scaled_width();
            const coord_t ext_perimeter_spacing = scale_(jconfig.hatch_spacing );//ext_perimeter_flow.scaled_spacing();
            ...
            for (Surfaces::iterator slice = layerm.slices.surfaces.begin();
                ...
                while (true) {
                    ...
                    const coord_t perimeters_thickness = ext_perimeter_width/2 + ext_perimeter_spacing/2
                        + (jconfig.perimeters-1 + slice->extra_perimeters) * perimeter_spacing;
   ...
   /* parallelize<Layer*>(
    ...
    parallelize<Slic3r::Layer*>(
    std::queue<Layer*>(std::deque<Slic3r::Layer*>(this->layers.begin(), this->layers.end())),
    [](Slic3r::Layer* layer) { layer->make_perimeters(); },
    this->jconfig.threads
);
...
parallelize<Slic3r::Layer*>(
    std::queue<Slic3r::Layer*>(std::deque<Layer*>(this->layers.begin(), this->layers.end())),
    [](Slic3r::Layer* layer) { layer->make_fills(); },
    this->_print->jconfig.threads
);
...
PrintObject::combine_infill()
{
    ...
    for (size_t region_id = 0; region_id < this->print()->regions.size(); ++ region_id) {
        ...
        const int every = 1;//region->config.infill_every_layers();
        if (every < 2 || region->jconfig.fill_density == 0.)
            continue;
        
        // Limit the number of combined layers to the maximum height allowed by this regions' nozzle.
        // FIXME: limit the layer height to max_layer_height
        const double nozzle_diameter =  this->_print->jconfig.beam_diameter;
        //std::min(
            //this->_print->jconfig.beam_diameter;//.get_at(region->config.infill_extruder.value - 1),
        ...
        for (size_t layer_idx = 0; layer_idx < combine.size(); ++layer_idx) {
            ...
            const float clearance_offset = region->jconfig.hatch_spacing *2;  // TODO: use region->config.infill_clearance
                    // This is the clearance offset for the infill, which is used to grow the fill areas.
                    // It is used to ensure that the fill areas overlap with perimeters and solid infill.
...
{
    std::cout << "Generating support material" << std::endl;
    ...
    if ((!jconfig.support_material && jconfig.anchors == 0)
            || this->layers.size() < 2
       ) {
    ...
    // Call the new support structure generation method for SLM printing
...
PrintObject::discover_horizontal_shells()
{
    ...
    for (size_t region_id = 0U; region_id < _print->regions.size(); ++region_id) {
        for (size_t i = 0; i < this->layer_count(); ++i) {
            ...
            const auto& region_config = layerm->region()->jconfig;

            //if (region_config.solid_infill_every_layers() > 0 && region_config.fill_density() > 0
               // && (i % region_config.solid_infill_every_layers()) == 0) 
                
                const auto type = this->jconfig.fill_density == 100 ? (stInternal | stSolid) : (stInternal | stBridge);
...
PrintObject::_discover_external_horizontal_shells(LayerRegion* layerm, const size_t& i, const size_t& region_id)
{
   // const auto& region_config = layerm->region()->config;
    for (auto& type : { stTop, stBottom, (stBottom | stBridge) }) {
        ...
        #endif
        size_t top_solid_layers=1;  // ignored for slm
        size_t bottom_solid_layers=1;
        size_t solid_layers = type == stTop
            ? top_solid_layers
            : bottom_solid_layers;
        solid_layers = std::min(solid_layers, this->layers.size());
          double min_top_bottom_shell_thickness = 2.0;
        if (min_top_bottom_shell_thickness > 0) {
            auto current_shell_thickness = static_cast<coordf_t>(solid_layers) * this->get_layer(i)->height;
            const auto min_shell_thickness =min_top_bottom_shell_thickness;
...
{    bool infill_only_where_needed=false;
    if (! infill_only_where_needed ||
        ! std::any_of(this->print()->regions.begin(), this->print()->regions.end(), 
            [](const PrintRegion *region) { return region->jconfig.fill_density > 0; }))
        return;
    ...
    for (int layer_id = int(this->layers.size()) - 1; layer_id > 0; --layer_id) {
        ...
        for (auto* layerm : lower_layer->regions) {
            if (layerm->region()->jconfig.fill_density == 0)
                continue;
...
void PrintObject::add_anchor_layers()
{
    // Get anchor parameters
    coordf_t first_layer_height = this->jconfig.first_layer_thickness;
    coordf_t anchor_thickness = this->jconfig.anchors_layer_thickness;
    coordf_t total_anchor_height = this->jconfig.anchors;
    
    // Early return if no anchors needed
    if (total_anchor_height <= 0) {
        return;
    }
    
    // Calculate number of anchor layers needed
    int num_anchor_layers = static_cast<int>(total_anchor_height / anchor_thickness);
    if (num_anchor_layers <= 0) {
        // If anchor height is too small, ensure at least one layer
        num_anchor_layers = 1;
    }
    
    // Step 1: Create a temporary vector to store the original layer pointers
    LayerPtrs original_layers;
    original_layers.reserve(layers.size());
    
    // Step 2: Copy all existing pointers to this temporary vector
    for (Layer* layer : this->layers) {
        original_layers.push_back(layer);
    }
    
    // Step 3: Clear the layers vector to rebuild it
    this->layers.clear();
    
    // Step 4: Reserve space for all layers (anchors + originals)
    this->layers.reserve(num_anchor_layers + original_layers.size());
    
    // Step 5: Create anchor layers and add them to the beginning
    Layer* prev_layer = nullptr;
    coordf_t current_height = 0;
    
    for (size_t i = 0; i < num_anchor_layers; i++) {
        // Calculate layer heights
        coordf_t layer_height = (i == 0) ? first_layer_height : anchor_thickness;
        current_height += layer_height;
        
        // Create an anchor layer with sequential ID starting from 0
        Layer* anchor_layer = this->add_layer(i, layer_height, current_height, 0);
        
        // Connect to previous layer if exists
        if (prev_layer != nullptr) {
            prev_layer->upper_layer = anchor_layer;
            anchor_layer->lower_layer = prev_layer;
        }
        
        // Add regions to anchor layer
        for (size_t region_id = 0; region_id < this->_print->regions.size(); ++region_id) {
            anchor_layer->add_region(this->print()->regions[region_id]);
        }
        
        prev_layer = anchor_layer;
    }
    
    // Step 6: Calculate the z-offset for original layers
    coordf_t anchor_z_offset = current_height;
    
    // Step 7: Add original layers back with updated IDs and z-heights
    for (size_t i = 0; i < original_layers.size(); i++) {
        Layer* original_layer = original_layers[i];
        
        // Update layer ID to follow anchor layers
        original_layer->set_id(num_anchor_layers + i);
        
        // Update print_z to account for anchor layers
        original_layer->print_z += anchor_z_offset;
        
        // Add to layers vector
        this->layers.push_back(original_layer);
        
        // Connect to previous layer
        if (prev_layer != nullptr) {
            prev_layer->upper_layer = original_layer;
            original_layer->lower_layer = prev_layer;
        }
        
        prev_layer = original_layer;
    }
    
    // The upper_layer of the last layer should be nullptr
    if (!this->layers.empty()) {
...
/**
 * Generates support structures for SLM printing
 * Identifies overhanging areas and creates support pillars
 */
void PrintObject::generate_support_structure()
{
    // Early exit if support is disabled
    if (!this->jconfig.support_material) {
        return;
    }
    
    // Get key parameters from config
    const coordf_t pillar_spacing = scale_(this->jconfig.support_material_spacing);
    const coordf_t pillar_size = scale_(this->jconfig.support_material_pillar_size);
    //const coordf_t contact_distance = scale_(this->jconfig.support_material_contact_distance);
    const double support_angle = this->jconfig.support_material_angle ;
    const double support_angle_rad = support_angle * M_PI / 180.0;
    
    // Collection to store overhanging areas by region and layer
    std::map<size_t, std::map<size_t, ExPolygons>> overhang_areas;
    
    // Detect overhanging areas in each layer
    detect_overhangs(overhang_areas, support_angle);
    
    // Generate support pillar positions
    std::map<size_t, Points> pillar_positions;
    generate_pillar_positions(overhang_areas, pillar_positions, pillar_spacing);
    
    // Create support pillars and add them to appropriate layers
    create_support_pillars(pillar_positions, pillar_size);
}

/**
 * Detect overhanging areas that need support
 */
void PrintObject::detect_overhangs(std::map<size_t, std::map<size_t, ExPolygons>>& overhang_areas, const double support_angle)
{
    const double tan_angle = tan(support_angle * M_PI / 180.0);
    
    // Skip the first layer (it's on the build plate)
    #pragma omp parallel for
    for (size_t layer_idx = 1; layer_idx < this->layers.size(); layer_idx++) {
        Layer* layer = this->layers[layer_idx];
        Layer* lower_layer = this->layers[layer_idx - 1];
        
        // Process each region separately
        for (size_t region_idx = 0; region_idx < layer->regions.size(); region_idx++) {
            LayerRegion* layerm = layer->regions[region_idx];
            LayerRegion* lower_layerm = lower_layer->regions[region_idx];
            
            // Skip if region is empty
            if (layerm->slices.empty()) continue;
            
            // Calculate how far can a layer extend over the lower one based on angle
            const coord_t max_support_distance = scale_(layer->height / tan_angle);
            
            // Get polygons for current layer
            ExPolygons current_polygons = layerm->slices;
            
            // Get polygons for lower layer
            ExPolygons lower_polygons = lower_layerm->slices;
            
            // Expand lower layer outline by max_support_distance
            ExPolygons supported_areas = offset_ex(lower_polygons, max_support_distance);
            
            // Areas of current layer not supported by expanded lower layer need support
            ExPolygons unsupported = diff_ex(current_polygons, supported_areas);
            
            if (!unsupported.empty()) {
                overhang_areas[region_idx][layer_idx] = unsupported;
            }
        }
    }
}
...
/**
 * Generate pillar positions based on overhang areas
 */
void PrintObject::generate_pillar_positions(
    const std::map<size_t, std::map<size_t, ExPolygons>>& overhang_areas,
    std::map<size_t, Points>& pillar_positions,
    const coordf_t pillar_spacing)
{
    // Process each region
    for (const auto& region_entry : overhang_areas) {
        size_t region_id = region_entry.first;
        Points region_pillars;
        
        // Collect all overhang areas for this region
        ExPolygons all_overhangs;
        for (const auto& layer_entry : region_entry.second) {
            all_overhangs.insert(all_overhangs.end(), 
                                 layer_entry.second.begin(), 
                                 layer_entry.second.end());
        }
        
        // Union all overhangs and apply slight offset to ensure proper support
        ExPolygons unified_overhangs = union_ex(all_overhangs);
        unified_overhangs = offset_ex(unified_overhangs, pillar_spacing / 3);
        
        // Create grid of points for each overhang area
        for (const ExPolygon& ex_poly : unified_overhangs) {
            BoundingBox bb = ex_poly.bounding_box();
            
            // Generate grid points within the bounding box
            for (coord_t y = bb.min.y; y < bb.max.y; y += pillar_spacing) {
                for (coord_t x = bb.min.x; x < bb.max.x; x += pillar_spacing) {
                    Point p(x, y);
                    
                    // Only add point if it's inside the overhang polygon
                    if (ex_poly.contains(p)) {
                        region_pillars.push_back(p);
                    }
                }
            }
        }
        
        // Store pillars for this region
        if (!region_pillars.empty()) {
            pillar_positions[region_id] = region_pillars;
        }
    }
}

/**
 * Create support pillar structures and add them to layers
 */
void PrintObject::create_support_pillars(
    const std::map<size_t, Points>& pillar_positions,
    const coordf_t pillar_size)
{
    // Store pillars by region using the class-defined SpPillar type, not the local Pillar
    std::map<size_t, std::vector<SpPillar>> pillars_by_region;
    
    // For each region, determine pillar heights
    for (const auto& region_entry : pillar_positions) {
        size_t region_id = region_entry.first;
        const Points& points = region_entry.second;
        std::vector<SpPillar> region_pillars;
        ...
        for (const Point& p : points) {
            ...
            pillar.position = p;
            pillar.top_layer = 0;
            pillar.bottom_layer = 0;
            pillar.radius_base = this->jconfig.support_material_pillar_size / 2.0;
            pillar.radius_top = pillar.radius_base * 0.8;
            
            bool object_hit = false;
            
            // Check layers top-down
            for (int i = this->layers.size() - 1; i >= 0; --i) {
                // Only process if the region exists in this layer
                if (region_id >= this->layers[i]->regions.size()) {
                    continue;
                }
                
                LayerRegion* layerm = this->layers[i]->regions[region_id];
                
                // Check if point is in void (not in filled area)
                // Replace the .contains() method with correct point-in-polygon check
                ...
                if (!point_inside) {
                    ...
                    if (pillar.top_layer > 0) {
                        // Extend existing pillar
                        pillar.bottom_layer = i;
                    } else if (object_hit) {
                        // Start new pillar below an object
                        pillar.top_layer = i;
                        object_hit = false;
                    }
                } else {
                    // Point is in filled area
                    if (pillar.top_layer > 0) {
                        // End of pillar, store it and start a new one
                        region_pillars.push_back(pillar);
                        // Reset pillar
                        pillar.position = p;
                        pillar.top_layer = 0;
                        pillar.bottom_layer = 0;
        ...
        // Store pillars for this region
        if (!region_pillars.empty()) {
            pillars_by_region[region_id] = region_pillars;
        }
    }
    
    // Create actual support structures in each layer
    generate_support_geometry(pillars_by_region, pillar_size);
}

/**
 * Generate support geometry for each layer based on pillar information
 */
void PrintObject::generate_support_geometry(
    const std::map<size_t, std::vector<SpPillar>>& pillars_by_region,
    const coordf_t pillar_base_size)
{
    // First, ensure the SurfaceType is defined correctly
    // If not already defined somewhere, define it here:
    // enum SurfaceType { stTop = 1, stBottom = 2, stInternal = 4, stSolid = 8, stBridge = 16, stVoid = 32, stSupport = 64 };
    ...
    for (size_t layer_idx = 0; layer_idx < this->layers.size(); layer_idx++) {
        ...
        // Process each region
        for (const auto& region_entry : pillars_by_region) {
            size_t region_id = region_entry.first;
            const std::vector<SpPillar>& pillars = region_entry.second;
            
            // Skip if region doesn't exist in this layer
            ...
            // Get pillar parameters for this region
            const coordf_t base_radius = this->jconfig.support_material_pillar_size / 2.0;
            const coordf_t top_radius = base_radius * 0.2; // Slightly smaller at top
            const coordf_t taper_height = this->jconfig.support_material_max_layers * layer.height;
            
            // Support polygons for this layer/region
            ExPolygons support_polys;
            
            // Process each pillar
            for (const SpPillar& pillar : pillars) {
                // Skip if pillar doesn't exist in this layer
                if (layer_idx < pillar.bottom_layer || layer_idx > pillar.top_layer) {
                    continue;
                }
                
                // Calculate pillar radius based on position in pillar height
                double height_position = 
                    static_cast<double>(layer_idx - pillar.bottom_layer) / 
                    static_cast<double>(pillar.top_layer - pillar.bottom_layer);
                
                // Taper pillar - thinner at top, wider at bottom
                double radius;
                if (height_position < 0.2) {
                    // Full size at bottom 20%
                    radius = base_radius;
                } else if (height_position > 0.8) {
                    // Smallest at top 20%
                    radius = top_radius;
                } else {
                    // Linear taper in the middle 60%
                    radius = base_radius - (base_radius - top_radius) * ((height_position - 0.2) / 0.6);
                }
                
                // Create pillar geometry
                const coord_t scaled_radius = scale_(radius);
                const Point& p = pillar.position;
                
                // Create circular pillar (approximated as polygon)
                Polygon circle;
                const int sides = 8 + 2 * (radius / 0.1); // More sides for larger pillars
                circle.points.reserve(sides);
                
                for (int i = 0; i < sides; i++) {
                    const double angle = 2.0 * M_PI * i / sides;
                    circle.points.emplace_back(
                        p.x + scaled_radius * cos(angle),
                        p.y + scaled_radius * sin(angle)
                    );
                }
                
                support_polys.emplace_back(ExPolygon(circle));
            }
            
            // Union support polygons and remove any that intersect with the current layer geometry
            if (!support_polys.empty()) {
                support_polys = union_ex(support_polys);
                
                // Avoid supports where they would touch the model (add small gap)
                const coord_t clearance = scale_(this->jconfig.support_material_model_clearance);
                
                // Extract surfaces to polygons correctly 
                ExPolygons fill_areas;
                for (const Surface& surface : layerm->fill_surfaces.surfaces) {
                    fill_areas.push_back(surface.expolygon);
                }
                
                ExPolygons model_with_clearance = offset_ex(fill_areas, clearance);
                
                // Remove support from model areas
                support_polys = diff_ex(support_polys, model_with_clearance, true);
                
                // Add to layer's support surfaces - use the correct surface type constant
                if (!support_polys.empty()) {
                    // Use stInternal + some flag to indicate support, or define stSupport in SurfaceType enum
                    layerm->support_surfaces.set(support_polys, stSupport);  // temporary: use stInternal instead of stSupport
                }
            }
        }
    }
