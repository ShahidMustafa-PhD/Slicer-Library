...
public:
    ...
    virtual ExtrusionRole get_role() const { return erNone; };
};
...
//    ExtrusionPath(ExtrusionRole role, const Flow &flow) : role(role), mm3_per_mm(flow.mm3_per_mm()), width(flow.width), height(flow.height) {};
    ...
    ExtrusionRole get_role() const { return this->role; }
    private:
...
class ExtrusionLoop : public ExtrusionEntity
{
    ...
    ExtrusionRole get_role() const {
        if (this->paths.empty()) return erNone;
        if (this->paths.size() > 1) 
        return this->paths.front().role;
    };
